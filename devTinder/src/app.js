...
// when we make a request to profile API, it will first go to the userAuth middleware, which will check if the user is authenticated or not, if the user is authenticated, it will call the next middleware function to get the data from the database and send it back to the client, if the user is not authenticated, it will send a response back to the client with an error message and status code 401 (Unauthorized)
...
// this api will only be called when the token is valid
